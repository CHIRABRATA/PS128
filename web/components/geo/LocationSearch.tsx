              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-xs font-bold text-emerald-950">
                    {isGpsOrigin ? "✓ Current location detected" : "✓ Location Selected"}
                  </span>
                  {selectedLocation.isUrban && (
                    <Badge variant="outline" className="text-[10px] px-1.5 py-0 bg-stone-100 text-stone-700 border-stone-300">
                      Urban Area
                    </Badge>
                  )}
                </div>
                <p className="text-sm font-semibold text-stone-900 mt-0.5">
                  {selectedLocation.displayName || selectedLocation.villageName || selectedLocation.placeName}
                </p>
                {(selectedLocation.blockName || selectedLocation.districtName) && (
                  <p className="text-xs text-stone-600 mt-0.5">
                    {[selectedLocation.blockName, selectedLocation.districtName, "Maharashtra"]
                      .filter(Boolean)
                      .join(", ")}
                  </p>
                )}
